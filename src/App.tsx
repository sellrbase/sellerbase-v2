      ...
      <main className="main">
        ...
        {selectedItem ? (
          ...
          <>
            ...
            {page === "buying" && <BuyingCalculator platforms={platforms} />}
            {page === "targets" && <TargetsPage items={items} sales={sales} />}
            {page === "analytics" && <AnalyticsPage items={items} sales={sales} expenses={expenses} />}
            {page === "tax" && <TaxPage sales={sales} expenses={expenses} />}
            {page === "settings" && (
              <SettingsPage business={business} platforms={platforms} onRefresh={refresh} />
            )}
          </>
        )}
      </main>
  ...
  return (
    <div className="page-stack">
      <div className="stat-grid">
        ...
        <Stat label="Open notices" value={notices.length} icon={<Bell />} />
      </div>
      ...
      <div className="split-grid">
        ...
        <section className="panel">
          ...
          <div className="mini-list">
            <p>{expenses.filter((expense) => expense.due_date && expense.due_date <= today()).length} bills due now</p>
            <p>{items.filter((item) => item.cost_each == null).length} items missing cost price</p>
            <p>{items.filter((item) => item.status === "Ready To List").length} items ready to list</p>
          </div>
        </section>
      </div>
    </div>
...
function AddStockPage({
  ...
  return (
    <section className="panel form-panel">
      ...
      <button className="primary-button" disabled={busy || !form.title || !form.list_price_each} onClick={save}>
...
function CompChecker() {
  ...
  return (
    <section className="panel form-panel">
      ...
      <div className="quick-grid">
        <a className={`quick-card ${!query ? "disabled" : ""}`} href={query ? `https://www.ebay.co.uk/sch/i.html?_nkw=${encoded}&LH_Sold=1&LH_Complete=1` : undefined} target="_blank" rel="noreferrer">eBay sold</a>
        <a className={`quick-card ${!query ? "disabled" : ""}`} href={query ? `https://www.ebay.co.uk/sch/i.html?_nkw=${encoded}` : undefined} target="_blank" rel="noreferrer">eBay active</a>
        <a className={`quick-card ${!query ? "disabled" : ""}`} href={query ? `https://www.vinted.co.uk/catalog?search_text=${encoded}` : undefined} target="_blank" rel="noreferrer">Vinted</a>
        <a className={`quick-card ${!query ? "disabled" : ""}`} href={query ? `https://www.depop.com/search/?q=${encoded}` : undefined} target="_blank" rel="noreferrer">Depop</a>
      </div>
    </section>
  );
}
...
function TargetsPage({ items, sales }: { items: InventoryItem[]; sales: Sale[] }) {
  const month = today().slice(0, 7);
  const monthSales = sales.filter((sale) => !sale.voided_at && sale.sold_at.startsWith(month));
  const revenue = monthSales.reduce((sum, sale) => sum + sale.sold_price_each * sale.quantity_sold, 0);
  const listed = items.filter((item) => item.created_at.startsWith(month)).length;
  return (
    <div className="page-stack">
      <section className="panel">
        <p className="eyebrow">This month</p>
        <h2>Targets</h2>
        <p className="muted">Saved custom targets are in the database plan; this first version shows the live numbers that target rules will use.</p>
      </section>
      <div className="stat-grid">
        <Stat label="Revenue" value={gbp(revenue)} icon={<CircleDollarSign />} />
        <Stat label="Items sold" value={monthSales.reduce((sum, sale) => sum + sale.quantity_sold, 0)} icon={<ShoppingBag />} />
        <Stat label="New stock added" value={listed} icon={<PackagePlus />} />
        <Stat label="Active listings" value={items.filter((item) => item.quantity_available > 0).length} icon={<Boxes />} />
      </div>
    </div>
  );
...
function BuyingCalculator({ platforms }: { platforms: PlatformSetting[] }) {
  const [platform, setPlatform] = useState(platforms[0]?.name || "eBay");
  ...
  return (
    <section className="panel form-panel">
      ...
      <p className="muted">{profit > 0 ? "Looks profitable on these assumptions." : "This looks too tight unless the sale price is higher or costs are lower."}</p>
    </section>
  );
}
  ...
  return (
    <div className="page-stack">
      <section className="panel">
        ...
        <h2>{business.name}</h2>
        <p className="muted">SKU sequence: {business.sku_prefix}{String(business.sku_start).padStart(3, "0")} onwards</p>
      </section>
      <section className="panel">
